...
// Landsat 5 SR Imagery — 4 Classes (1990)
...
// ============================================================
// STEP 2: LOAD AND FILTER LANDSAT 5 DATA
// ============================================================

var startDate = '1990-06-01';
var endDate   = '1990-08-31';
...
print('Total Landsat 5 Scenes:', collection.size());

var composite = collection.median().clip(aoi);

// Visualize True Color (RGB = B3, B2, B1)
...
var ndwi = composite.normalizedDifference(['SR_B2', 'SR_B4']).rename('NDWI');

// MNDWI — Modified water index (Xu)
var mndwi = composite.normalizedDifference(['SR_B2', 'SR_B5']).rename('MNDWI');

// NDBI — Built-up index
var ndbi = composite.normalizedDifference(['SR_B5', 'SR_B4']).rename('NDBI');
...
var bsi = composite.expression(
  '((SWIR1 + RED) - (NIR + BLUE)) / ((SWIR1 + RED) + (NIR + BLUE))',
  {
    ...
    'SWIR1': composite.select('SR_B5')
  }
).rename('BSI');

// UI — Urban Index
var ui_index = composite.normalizedDifference(['SR_B5', 'SR_B3']).rename('UI');

// Stack all bands + indices into one multi-band image
var inputBands = ['SR_B1', 'SR_B2', 'SR_B3', 'SR_B4', 'SR_B5', 'SR_B7'];
...
//   TIP: For Riyadh 1990, Water is rare — focus on seasonal streams/reservoirs.
// ============================================================
...
Map.addLayer(classified.clip(aoi), classVis, 'LULC Classification — 1990');
...
Export.image.toDrive({
  ...
  description:    'LULC_Riyadh_1990_RF',
  folder:         'Riyadh',
  fileNamePrefix: 'LULC_Riyadh_1990_RF',
  region:         aoi,
  scale:          30,
  maxPixels:      1e13,
  crs:            'EPSG:4326'
...
// Export.image.toDrive({
//   image:          ndvi.clip(aoi),
//   description:    'NDVI_Riyadh_1990',
//   folder:         'GEE_Exports',
//   fileNamePrefix: 'NDVI_Riyadh_1990',
//   region:         aoi,
//   scale:          30,
//   maxPixels:      1e13,
//   crs:            'EPSG:4326'
